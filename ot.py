import hashlib, json, logging, pickle, util, yao
...
class ObliviousTransfer:
    ...
    def send_result(self, circuit, g_tables, pbits_out, b_inputs):
        ...
        # Print Bob's result to file
        with open('output/Bob_result.json', 'w', encoding='utf-8') as f:
            json.dump({'bob': {'wires': ' '.join(map(str, list(b_inputs.keys()))), 'binary': ' '.join(map(str, list(b_inputs.values()))), 'decimalValue': int(''.join(list(map(str, b_inputs.values()))), 2)}, 'result': {'wires': ' '.join(map(str, list(result.keys()))), 'binary': ' '.join(map(str, list(result.values()))), 'decimalValue': int(''.join(list(map(str, result.values()))), 2)}}, f, ensure_ascii=False, indent=4)
       
        # Print what Bob sends to Alice to file
        with open('output/Bob_send_to_alice.json', 'w', encoding='utf-8') as f:
    ...
    def ot_garbler(self, msgs):
        ...
        # Print Alice OT to file
        with open('output/Alice_OT.json', 'w', encoding='utf-8') as f:
            json.dump(str((c1, e0, e1)), f, ensure_ascii=False, indent=4)
    ...
    def ot_evaluator(self, b):
        ...
        # Print Bob OT to file
        with open('output/Bob_OT.json', 'w', encoding='utf-8') as f:
            json.dump(str(mb), f, ensure_ascii=False, indent=4)
